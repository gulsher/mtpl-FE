...
export default class AssignedChannelPage extends Component {
  ...
  addChannelToPkg = async () => {
    ...
    let resp = await loginService.addChannelToPkg(
      this.state.PackageId,
      payload
    );
    if (resp) {
  ...
  removeChannelfromPkg = async id => {
    ...
    let resp = await loginService.removeChannelfromPkg(
      this.state.PackageId,
      payload
    );
    if (resp) {
                    ...
                    ? this.state.assignedChannelList.map((item, key) => {
                        return (
                          <tr key={key}>
                            ...
                            <td>
                              {this.state.displaySubPackage
                                ? this.state.displaySubPackage.packName
                                : "select Subpackage"}
                            </td>
                            ...
                            <td>
                              <button
                                ...
                                onClick={e =>
                                  this.removeChannelfromPkg(item.id)
                                }
                              >
                                Remove
                              </button>
                  ...
                  /> */}
                      <ul id="channelFilter">
                        {this.state.notAssignedChannelList.length > 0
                          ? this.state.notAssignedChannelList.map(
                              (item, key) => {
                                return (
                                  <li key={key}>
                                    <div className="custom-control custom-checkbox mb-3 mx-1">
                                      <input
                                        ...
                                        onChange={e =>
                                          this.assignHandler(e, item)
                                        }
                                        checked={this.state.channelIDs.includes(
                                          item.id
                                        )}
                                      />
                                      <img
